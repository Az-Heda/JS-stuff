class Logger {
	...
	static default(text, lv = 1) {
		...
		console.log(`%cLogger.${this.#_prepareName('default')} - ${this.#_getDate()}`, this.#_getStyles('#C084FC', '#000000'), text);
	}
	...
	static add(name, bg, color = '#000000') {
		...
		Object.defineProperty(this, this.#_prepareName(name), {
			value: (text, lv = 1) => {
				if (lv <= this.#_level) return;
				console.log(`%cLogger.${this.#_prepareName(name)} - ${this.#_getDate()}`, this.#_getStyles(bg, color), text);
			},
			// writable: false,
			// configurable: false,
		})
