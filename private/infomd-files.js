...
const idir = 'Client/CachedFunction';
...
function MDCreator(p, filename, data) {
	const title = {};
	new Array(6).fill(0).map((e, i) => {
		title[`h${i+1}`] = (t) => `${''.padStart(i+1, '#')} ${t}` ;
	})
	let lines = ['__This file is autogenerated__'];
	...
	data.new.forEach((el, i) => {
		if (el.indexOf('#') === -1 && originals[i].indexOf('@no_docs') == -1) {
	...
	fs.writeFile(path.join(p, filename), lines.join('\n'), () => {});
}
...
function writeMD(p, data) {
	...
	.map((item) => {
		const functionParser = (finit) => {
			...
			if (doesReturnSomething) {
				...
				const returnString = ` -> ${returnType.substr(1, returnType.length - 2)}`;
				text += returnString
				if (finit === baseName) return baseName + returnString;
			}
			if (finit === baseName) return baseName
			return text;
		};
	...
	MDCreator('auto-docs', `Auto-${p.replaceAll('\\', '/').split('/').splice(-1)[0]}.md`, newObject);
...
fs.readdir(clientDir, (err, files) => {
	...
	files = files.filter((file) => { return fs.lstatSync(file).isDirectory() })
	console.log(files);
